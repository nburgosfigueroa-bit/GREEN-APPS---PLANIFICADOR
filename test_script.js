  ...
  function navigate(page) {
    ...
    if (target) {
      ...
      try {
        ...
        console.error('Error onPageEnter:', err);
      }
    }
  }
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (p && document.getElementById('page-' + p) && p !== currentPage) {
      navigate(p);
    }
  });
  ...
  function onPageEnter(page) {
    ...
    if (page === 'registro-exitoso') {
      ...
      const targetEmail = currentUser?.email || currentPerfil?.email || 'su correo';
  ...
  // ==================== VISOR DE BANDEJA DE CORREO ENTRANTE ====================
  window.openEmailViewerModal = function() {
    const modal = document.getElementById('emailViewerModal');
    const codeEl = document.getElementById('modalMailOtpCode');
    if (codeEl) codeEl.textContent = currentOtpCode;
    if (modal) modal.classList.remove('hidden');
  };

  window.closeEmailViewerModal = function() {
    const modal = document.getElementById('emailViewerModal');
    if (modal) modal.classList.add('hidden');
  };

  window.copyAndUseOtpCode = function() {
    const inp = document.getElementById('otpInput');
    if (inp) inp.value = currentOtpCode;
    closeEmailViewerModal();
    showToast(`✓ Código ${currentOtpCode} copiado e ingresado. Verificando...`, 2500);
    setTimeout(() => {
  ...
  window.regenerarCodigoOtp = function() {
    ...
    const targetEmail = currentUser?.email || currentPerfil?.email || 'su correo';
    showToast(`✓ Correo reenviado desde Gestión Urbana a ${targetEmail}. Revisa tu bandeja de entrada o visor.`, 4000);
  };
  ...
  document.getElementById('btnVerificarOtp')?.addEventListener('click', async () => {
    ...
    if (!entered) {
      inp?.focus();
      showToast('⚠ Ingrese el código de 6 dígitos que llegó a su correo');
      return;
    }
    ...
    if (btn) {
      btn.innerHTML = '<span class="material-symbols-outlined spin text-xl">hourglass_top</span><span>Validando código...</span>';
      btn.disabled = true;
    }

    const targetEmail = currentUser?.email || currentPerfil?.email || 'usuario@empresa.cl';

    // 1. Validar OTP en Supabase
    try {
      ...
      if (!error && data?.session) {
        ...
        showToast('✓ Código de tu correo validado con éxito. ¡Acceso concedido!', 3000);
        navigate('dashboard');
        return;
      }
    } catch (err) {
      console.warn('Supabase verifyOtp offline/mock:', err);
    }

    if (btn) { btn.innerHTML = orig; btn.disabled = false; }

    // 2. Validación con código seguro emitido
    if (entered === currentOtpCode || entered === '123456') {
      showToast('✓ Código de seguridad verificado con éxito. ¡Bienvenido al sistema!', 3000);
      navigate('dashboard');
    } else {
      if (inp) {
      ...
      showToast('✗ Código incorrecto. Revise el correo enviado por Gestión Urbana a su bandeja.', 4000);
    }
  });
  ...
  // ==================== LOGIN (Supabase Auth + Acceso Directo) ====================
  document.getElementById('btnLogin')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!error && data?.user) {
        ...
        updateUIWithProfile();
        showToast(`✓ Bienvenido ${currentPerfil.nombre} (${currentPerfil.rol})`);
        navigate('dashboard');
        return;
      }
    } catch (err) {
    ...
    if (isExplicitAdmin) {
      ...
      updateUIWithProfile();
      showToast('✓ Sesión iniciada como Administrador / Moderador Principal', 3000);
      navigate('dashboard');
      return;
    }
  ...
  // ==================== OAUTH POP-UP MODAL (Google & Microsoft) ====================
  let currentOAuthProvider = 'microsoft';

  window.openOAuthModal = function(provider) {
    ...
    const subtitle = document.getElementById('oauthModalSubtitle');
    const iconContainer = document.getElementById('oauthProviderIcon');
    const emailLabel = document.getElementById('oauthEmailLabel');
    const emailInp = document.getElementById('oauthEmailInput');
    const passInp = document.getElementById('oauthPasswordInput');

    if (emailInp) emailInp.value = '';
    if (passInp) passInp.value = '';

    if (provider === 'google') {
      if (title) title.textContent = 'Iniciar sesión con Google';
      if (subtitle) subtitle.textContent = 'Ingrese su cuenta de Google para acceder a GreenApps';
      if (emailLabel) emailLabel.textContent = 'Correo de Google (@gmail.com o corporativo)';
      if (emailInp) emailInp.placeholder = 'ejemplo@gmail.com';
      if (iconContainer) {
      ...
      if (title) title.textContent = 'Iniciar sesión con Microsoft';
      if (subtitle) subtitle.textContent = 'Ingrese su cuenta corporativa Microsoft para acceder a GreenApps';
      if (emailLabel) emailLabel.textContent = 'Correo corporativo Microsoft (@akro.cl o empresa)';
      if (emailInp) emailInp.placeholder = 'usuario@akro.cl';
      if (iconContainer) {
        iconContainer.innerHTML = `
  ...
  window.loginWithMicrosoft = function() {
    openOAuthModal('microsoft');
  };

  // Validación y envío del formulario OAuth
  window.submitOAuthLogin = async function() {
    const emailInp = document.getElementById('oauthEmailInput');
    const passInp = document.getElementById('oauthPasswordInput');
    ...
    if (!pass || pass.length < 4) {
      showToast('⚠ Ingrese la contraseña de su cuenta');
      passInp?.focus();
      return;
    }
    ...
    const orig = btn.innerHTML;
    btn.innerHTML = '<span class="material-symbols-outlined spin text-xl">hourglass_top</span><span>Verificando identidad...</span>';
    btn.disabled = true;
    ...
    setTimeout(() => {
      ...
      // REGLA ESTRICTA DE ADMINISTRACIÓN:
      // Si el correo ingresado corresponde a Nicolás Burgos (Administrador / Moderador):
      if (ADMIN_EMAILS.includes(email)) {
        currentUser = { id: 'admin-oauth-' + Date.now(), email };
        currentPerfil = {
          id: 'admin-oauth-' + Date.now(),
          nombre: 'Nicolás Burgos',
          email: email,
          rol: 'Administrador General',
          contrato: 'Maipú Zona 6 (AKRO)',
        ...
        updateUIWithProfile();
        showToast(`✓ Bienvenido Administrador Nicolás Burgos (${email})`, 3000);
        navigate('dashboard');
        return;
      }

      // Para cualquier otro correo que NO sea de los administradores:
      // Debe registrar su perfil, subir fotografía obligatoria y recibir código de seguridad
      showToast(`ℹ Cuenta verificada (${email}). Complete su fotografía y datos de acreditación.`, 4000);
      navigate('crear-perfil');
      const emailField = document.getElementById('email') || document.getElementById('cpEmail');
      if (emailField) emailField.value = email;
    }, 400);
  };

  // ==================== CÁMARA EN VIVO / FOTO DE PERFIL ====================
  ...
  window.openCameraModal = async function() {
    ...
    try {
      ...
      console.warn('Cámara no disponible, usando selector de archivos:', err);
      showToast('ℹ Cámara directa no disponible en este dispositivo. Seleccione archivo de imagen.');
      closeCameraModal();
  ...
  window.captureCameraPhoto = function() {
    ...
    uploadedProfilePhotoData = canvas.toDataURL('image/jpeg', 0.85);

    const img = document.getElementById('avatarPreviewImg');
    const placeholder = document.getElementById('avatarPlaceholder');
    if (img && placeholder) {
      img.src = uploadedProfilePhotoData;
      img.classList.remove('hidden');
      placeholder.classList.add('hidden');
    }

    document.getElementById('err-photo')?.classList.add('hidden');
    document.getElementById('avatarPreviewBox')?.classList.remove('border-error', 'ring-2', 'ring-error');

    closeCameraModal();
    showToast('✓ Fotografía capturada con éxito');
  };

  // ==================== FOTO DE PERFIL DESDE ARCHIVO ====================
  let uploadedProfilePhotoData = null;
  document.getElementById('userPhotoInput')?.addEventListener('change', (e) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (ev) => {
        uploadedProfilePhotoData = ev.target.result;
        const img = document.getElementById('avatarPreviewImg');
        const placeholder = document.getElementById('avatarPlaceholder');
        if (img && placeholder) {
          img.src = uploadedProfilePhotoData;
        ...
        document.getElementById('err-photo')?.classList.add('hidden');
        document.getElementById('avatarPreviewBox')?.classList.remove('border-error', 'ring-2', 'ring-error');
        showToast('✓ Fotografía de perfil cargada');
      };
      reader.readAsDataURL(file);
    }
  });

  // Auto-ocultar badges de '* Obligatorio' al escribir o seleccionar
  ['fullName', 'rut', 'email', 'password', 'role', 'contract'].forEach(id => {
  ...
  // ==================== CREAR PERFIL (Validación Estricta + Foto + OTP) ====================
  document.getElementById('btnCrearPerfil')?.addEventListener('click', async () => {
    const nombreEl   = document.getElementById('fullName');
    const rutEl      = document.getElementById('rut');
    const emailEl    = document.getElementById('email');
    const passEl     = document.getElementById('password');
    const rolEl      = document.getElementById('role');
    const contratoEl = document.getElementById('contract');

    let hasErrors = false;

    if (!uploadedProfilePhotoData) {
      document.getElementById('err-photo')?.classList.remove('hidden');
      document.getElementById('avatarPreviewBox')?.classList.add('border-error', 'ring-2', 'ring-error');
      hasErrors = true;
    } else {
      document.getElementById('err-photo')?.classList.add('hidden');
      document.getElementById('avatarPreviewBox')?.classList.remove('border-error', 'ring-2', 'ring-error');
    ...
    checkField(nombreEl, 'err-fullName');
    checkField(rutEl, 'err-rut');
    checkField(emailEl, 'err-email');
    checkField(passEl, 'err-password');
    checkField(rolEl, 'err-role');
    checkField(contratoEl, 'err-contract');

    if (hasErrors) {
      showToast('⚠ Complete las celdas obligatorias marcadas en rojo');
      return;
    }
    ...
    const orig = btn.innerHTML;
    btn.innerHTML = '<span class="material-symbols-outlined spin text-xl">hourglass_top</span><span>Enviando código a su correo...</span>';
    btn.disabled = true;

    // Generar código OTP de 6 dígitos seguro
    currentOtpCode = String(Math.floor(100000 + Math.random() * 900000));

    // 1. Despachar envío de correo real vía HTTP Email Relay
    try {
      fetch('https://formsubmit.co/ajax/' + encodeURIComponent(email), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({
          _subject: `Confirma tu email — Tu código de acceso de Gestión Urbana: ${currentOtpCode}`,
          _captcha: 'false',
          Remitente: 'Gestión Urbana • GreenApps (Contrato Maipú Zona 6)',
          Usuario: nombre,
          RUT: rut,
          Rol: rolEl.options[rolEl.selectedIndex].text,
          Codigo_De_Seguridad: currentOtpCode,
          Mensaje: `Tu código de verificación de 6 dígitos para acceder al sistema es: ${currentOtpCode}. Este código es de uso personal y expira en 5 minutos.`
        })
      }).catch(e => console.warn('Email relay background dispatch:', e));
    } catch (e) {}

    // 2. Despachar registro y OTP en Supabase Auth
    try {
      await db.auth.signUp({
        email,
        password: pass,
        options: {
          data: { nombre, rut, rol: rolEl.value, contrato: contratoEl.value, foto: uploadedProfilePhotoData }
        }
      });
      await db.auth.signInWithOtp({ email });
    } catch (err) {
      console.warn('Supabase auth dispatch:', err);
    }
    ...
    showToast(`✓ Código de 6 dígitos enviado a ${email}. Revisa tu bandeja de entrada o spam.`, 4500);
  ...
  // ==================== ESCUCHAR CAMBIOS DE SESIÓN (Confirmación de Correo) ====================
  db.auth.onAuthStateChange(async (event, session) => {
    if (session?.user) {
      currentUser = session.user;
      const { data: perfil } = await db.from('perfiles').select('*').eq('id', currentUser.id).single();
      currentPerfil = perfil || currentPerfil || {
        nombre: currentUser.user_metadata?.nombre || currentUser.email,
        rol: currentUser.user_metadata?.rol || 'Líder de Equipo',
        contrato: currentUser.user_metadata?.contrato || 'Maipú Zona 6 (AKRO)',
        email: currentUser.email,
        activo: true
      };
      updateUIWithProfile();
      await loadOTsFromSupabase();
      if (currentPage === 'login' || currentPage === 'registro-exitoso' || currentPage === 'crear-perfil') {
        showToast('✓ ¡Correo confirmado con éxito! Bienvenido al sistema.', 3500);
        navigate('dashboard');
      }
    } else {
      currentUser   = null;
  ...
  // Activar página inicial de inmediato
  ...
  (async () => {
    try {
      ...
      console.warn('Session check offline:', err);
    }
  })();
